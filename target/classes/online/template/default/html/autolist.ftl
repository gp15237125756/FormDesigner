	...
	columns:[
		[	
			...
			{field:'opt',title:'操作',width:200,formatter:function(value,rec,index){
						<#assign primaryKeyIndex=fields?index_of(",",1)>
						if(!rec.${fileds?substring(0,primaryKeyIndex)}){return '';}
						var href='';
						<#if config_noliststr?index_of("delete")==-1>
						<#--//update-begin--Author:zhangjiaqiang  Date:20160925 for：TASK #1344 [链接图标] online功能测试的按钮链接图标修改 -->
						href+="<a  href='javascript:void(0)' class='ace_button' onclick=delObj('cgAutoListController.do?del&configId=${config_id}&${fileds?substring(0,primaryKeyIndex)}="+rec.${fileds?substring(0,primaryKeyIndex)}+"','${config_id}List')>";
