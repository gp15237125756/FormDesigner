...
function createDataGrid${config_id}(){
	...
	columns:[
		[	
			...
			{field:'opt',title:'操作',width:200,formatter:function(value,rec,index){
						<#assign primaryKeyIndex=fileds?index_of(",",1)>
						if(!rec.${fileds?substring(0,primaryKeyIndex)}){return '';}
						var href='';
						<#if config_noliststr?index_of("delete")==-1>
						<#-- //update-begin--Author:zhangdaihao  Date:20160925 for：TASK #1344 [链接图标] online功能测试的按钮链接图标修改 -->
						href+="<a href='javascript:void(0)' class='ace_button' onclick=delObj('cgAutoListController.do?del&configId=${config_id}&pkName=${fileds?substring(0,primaryKeyIndex)}&id="+rec.${fileds?substring(0,primaryKeyIndex)}+"','${config_id}List')>";
	...
	function ${config_id}add(){
		...
		add('<@mutiLang langKey="${config_name}"/>录入','cgFormBuildController/ftlForm/${config_id}/goAdd.do?olstylecode=${_olstylecode}&pkName=${fileds?substring(0,primaryKeyIndex)}','${config_id}List',${config_id}Fw,${config_id}Fh);
	...
	function ${config_id}update(){
		...
		_update('<@mutiLang langKey="${config_name}"/>编辑','cgFormBuildController/ftlForm/${config_id}/goUpdate.do?olstylecode=${_olstylecode}&pkName=${fileds?substring(0,primaryKeyIndex)}','${config_id}List','${fileds?substring(0,primaryKeyIndex)}',${config_id}Fw,${config_id}Fh);
	}
	//查看
