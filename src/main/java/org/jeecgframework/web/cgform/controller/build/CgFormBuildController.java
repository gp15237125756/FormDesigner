...
//			String mode=request.getParameter("mode");
	        ...
	        if(StringUtils.isNotEmpty(id)){
	        	...
	        	dataForm = dataBaseService.findOneForJdbc(tablename, id,pkName);
		    	...
		    	if(head.getJformType()==CgAutoListConstant.JFORM_TYPE_MAIN_TALBE){
			    	...
			    	if(StringUtils.isNotEmpty(subTableStr)){
			    		 ...
			    		 for(String subTable:subTables){
				    			...
				    			subTableData = cgFormFieldService.getSubTableData(tableName,subTable,pkName,id);
			...
			if(StringUtils.isNotBlank(urlTemplateName)){
				...
				System.out.println("urlTemplateName: "+urlTemplateName);
			}else{
			...
			Set<Entry<String,Object>> maps=data.entrySet();
			for(Entry<String,Object> e:maps){
				System.out.println("uopdate template data: "+e.getKey()+"-"+e.getValue());
			}
			System.out.println("uopdate template url: "+content);
...
//		sb.append("<script type=\"text/javascript\" src=\"plug-in/lhgDialog/lhgdialog.min.js\"></script>");
	...
	public AjaxJson saveOrUpdate(HttpServletRequest request) throws Exception{
		...
		if(data!=null){
			...
			String pkName = (String)data.get("pkName");
			//打印测试
		    ...
		    if(StringUtils.isEmpty(id)){
			    ...
			    try {
			    	...
			    	data.put("id", pkValue);
			    	/** 增加默认属性values*/
			    	PublicUtil.addCommonForTable(data);
				...
				//消除不是表的字段
			    String [] filterName = {"tableName","saveOrUpdate","id","pkName"};
			    data = CommUtils.attributeMapFilter(data,filterName);
			    PublicUtil.updateCommonForTable(data);
			    //更新数据库
			    try {
					//int num = dataBaseService.updateTable(tableName, id, data);
			    	//int num = dataBaseService.updateTable(tableName, id, data);
			    	int num = dataBaseService.updateTable(tableName, id,pkName, data);
					if (num>0) {
						j.setSuccess(true);
						message = "业务更新成功";
					}else {
		...
		if(data!=null){
		    ...
		    if(StringUtils.isEmpty(id)){
		    	...
		    	try {
		    		...
		    		Map<String, Object> result = dataBaseService.insertTableMore(mapMore, tableName,pkName);
				...
				try {
					...
					dataBaseService.updateTableMore(mapMore, tableName,pkName);
					j.setSuccess(true);
					message = "更新成功";
				} catch (BusinessException e) {
					e.printStackTrace();
