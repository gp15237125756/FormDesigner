...
public class DataBaseServiceImpl extends CommonServiceImpl implements DataBaseService{
	...
	public void insertTable(String tableName, Map<String, Object> data) throws BusinessException {
		...
		System.out.println("insert sql: "+sql+" data: "+data);
	...
	public int updateTable(String tableName, Object id,String pkName, Map<String, Object> data) throws BusinessException {
		...
		if(id instanceof java.lang.String){
			sqlBuffer.append(" where ").append(pkName).append(" ='").append(id).append("'");
		}else{
			sqlBuffer.append(" where ").append(pkName).append(" =").append(id);
		}
		System.out.println("update Sql: "+sqlBuffer.toString()+" data: "+data);
		CgFormHeadEntity cgFormHeadEntity = cgFormFieldService.getCgFormHeadByTableName(tableName);
		int num = this.executeSql(sqlBuffer.toString(), data);
...
//          sql = sql.replace("${" + key + "}", "'"+String.valueOf(params.get(key))+"'");
	...
	public Map<String, Object> insertTableMore(Map<String, List<Map<String, Object>>> mapMore, String mainTableName,String pkName) throws BusinessException {
	    ...
	    if(mainMap.get("id")==null||"".equals((String)mainMap.get("id"))){
		    ...
		    mainMap.put("id", pkValue);
		    /** 增加外键值*/
		    if(org.apache.commons.lang3.StringUtils.isNotEmpty(pkName)&&null!=pkValue)
		    mainMap.put(pkName, pkValue);
	    }
	    /** 增加默认属性values*/
    	PublicUtil.addCommonForTable(mainMap);
	    Object pk=getInsertTableKey(mainTableName, mainMap);
	    Object pkV=mainMap.get(pkName);
	    if(pkV==null||(pkV instanceof String&&StringUtils.isBlank((String)pkV))){
	    	mainMap.put(pkName, pk);
	    }
		//插入附表信息
		//去除主表信息
		String [] filterMainTable = {mainTableName};
		...
		while(it.hasNext()){
	        ...
	        for(Map<String, Object> fieldMap:ov){
	        	...
	        	fieldMap = CommUtils.convertFKMap(fieldMap,mainMap,fkFieldList);
	        	 /** 增加默认属性values*/
	        	PublicUtil.addCommonForTable(fieldMap);
	        	insertTable(ok, fieldMap);
	        }
		}
		return mainMap;
	...
	public boolean updateTableMore(Map<String, List<Map<String, Object>>> mapMore, String mainTableName) throws BusinessException {
	    ...
	    mainMap = CommUtils.attributeMapFilter(mainMap,filterName);
	    PublicUtil.updateCommonForTable(mainMap);
		...
		while(it.hasNext()){
	        ...
	        for(Map<String, Object> fieldMap:ov){
	        	...
	        	if(subId==null || "".equals(String.valueOf(subId))){
	        	    ...
	        	    PublicUtil.updateCommonForTable(fieldMap);
	...
	public boolean updateTableMore(Map<String, List<Map<String, Object>>> mapMore, String mainTableName,String pkName) throws BusinessException {
	    ...
	    /** update 2017-9-15*/
		//updateTable(mainTableName,mainTableId, mainMap);
		updateTable(mainTableName,mainTableId,pkName, mainMap);
		mainMap.put("id", mainTableId);
		//更新附表信息
		String [] filterMainTable = {mainTableName};
		mapMore = CommUtils.attributeMapFilter(mapMore,filterMainTable);
		...
		while(it.hasNext()){
        	...
        	List<Map<String, Object>> fkFieldList =  getFKField(mainTableName, ok);
        	/** 获取字表主键名称*/
        	String subPkName=getSubTablePkName(mainTableName);
        	//根据主表id获取附表信息
        	//Map<Object,Map<String, Object>> subTableDateMap = getSubTableData(fkFieldList, mainTableName, ok, mainTableId);
        	Map<Object,Map<String, Object>> subTableDateMap = getSubTableData(fkFieldList, mainTableName, ok, mainTableId,pkName,subPkName);
	        for(Map<String, Object> fieldMap:ov){
	        	Object subId = fieldMap.get(subPkName)==null?"":fieldMap.get(subPkName);
	        	//Object pkV=fieldMap.get(pkName);
	        	/** 判断id不存在*/
	        	if((subId==null || "".equals(String.valueOf(subId)))){
	        		fieldMap = CommUtils.convertFKMap(fieldMap,mainMap,fkFieldList);
		        	fieldMap.put("id", getPkValue(ok));
	        	    ...
	        	    fieldMap = CommUtils.attributeMapFilter(fieldMap,subFilterName);
	        		//updateTable(ok,subId,fieldMap);
	        		updateTable(ok,subId,subPkName,fieldMap);
	        		//剔除已经更新了的数据
	        		for(Iterator<Map.Entry<Object,Map<String, Object>>> i$=subTableDateMap.entrySet().iterator();i$.hasNext();){
	        			Map.Entry<Object,Map<String, Object>> i=i$.next();
	        			if(subId instanceof String){
		        			if(Objects.equals(Long.parseLong((String)subId), i.getKey())){
		        				//subTableDateMap.remove(Long.parseLong((String)subId));
		        				i$.remove();
		        			}
	        			}
	        		}
	        ...
	        if(subTableDateMap.size()>0){
		    	...
		    	while(itSub.hasNext()){
		    		...
		    		//deleteSubTableDataById(subId,ok);
		    		/** update 2017-9-15*/
		    		deleteSubTableDataById(subId,ok,subPkName);
		    	}
	        }
		}
		return true;
	 ...
	 * @return
	 */
	private Map<Object,Map<String, Object>> getSubTableData(List<Map<String, Object>> fkFieldList,String mainTableName, String subTableName,Object mainTableId,String pkName,String subPkName) {
		...
		if(subTableDataList!=null){
			for(Map<String,Object> map:subTableDataList){
				dataMap.put(map.get(subPkName), map);
			}
		}
		return dataMap;
	}
	 ...
	 * @return String
	 */
	private String getSubTablePkName(String mainTableName) {
		CgFormHeadEntity  cghead = cgFormFieldService.getCgFormHeadByTableName(mainTableName);
		String subTables=cghead.getSubTableStr(),subTableName="";
		if(StringUtils.isNotBlank(subTables)){
			subTableName= StringUtils.split(subTables, ",")[0];
		}
		//select f.field_name from  cgform_head h,cgform_field f where h.id=f.table_id and h.table_name='' and f.is_key='Y';
		StringBuilder sql2 = new StringBuilder("");
		sql2.append("select f.field_name from cgform_head h,cgform_field f where h.id=f.table_id and h.table_name='");
		sql2.append(subTableName).append("' and f.is_key='Y'");
		List<Map<String,Object>> subTableDataList = this.findForJdbc(sql2.toString());
		if(CollectionUtils.isNotEmpty(subTableDataList)){
			Map<String,Object> map=subTableDataList.get(0);
			return map.get("field_name")==null?null:(String)map.get("field_name");
		}
		return null;
	}
	 ...
	 * @return
	 */
	private void deleteSubTableDataById(Object subId,String subTableName,String pkName){
		StringBuilder sql = new StringBuilder("");
		sql.append(" delete from ").append(subTableName).append(" where ").append(pkName).append(" = ? ");
		this.executeSql(sql.toString(), subId);
	}
	/**
	 * 更新操作时将系统变量约定的字段赋值
